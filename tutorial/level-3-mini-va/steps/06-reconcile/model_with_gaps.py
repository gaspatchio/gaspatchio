"""
Level 3 → Step 06: Reconciliation Starting Model (with 4 deliberate gaps)

This model takes the L3 Step 05 logic and points it at Level 4's production
data. It adds the 3 data joins that L4 needs (product params, dynamic lapse
params, space params) and unpivots scenario returns from wide to long format.

Four DELIBERATE GAPS are embedded, marked with '=== GAP N' comments.
The student discovers these by reconciling against L4's reference output
and fixes them one at a time.

Gap summary:
  GAP 1 (SECTION 5):  AV via cum_prod — should use accumulate()
  GAP 2 (SECTION 6):  Simple decrement ordering — should use BEF_DECR pattern
  GAP 3 (SECTION 4):  DL001 only — should select by formula_id
  GAP 4 (SECTION 11): Discount factors via cum_prod — should use closed-form
"""
...
def main(af: ActuarialFrame) -> ActuarialFrame:
    """Main model projection with 4 deliberate gaps for reconciliation."""
    ...
    # === GAP 1: Account value via cum_prod — should use accumulate() ===
    # This merges growth and fee into one factor and uses cumulative product.
    # Works for IF business but cannot handle NB premium injection.
    # Fix: decompose into accumulate(initial, multiply, add) pattern.
    ...
    # =====================================================================
    # SECTION 4: DYNAMIC LAPSE
    # =====================================================================

    af.itm = af.av_pp_mid_mth / af.sum_assured.cast(pl.Float64)

    # === GAP 3: Single dynamic lapse formula — should select by formula_id ===
    # Only DL001 is implemented. GMAB products use DL002 (power function).
    # Fix: add when(formula_id == "DL001").then(dl001).otherwise(dl002).
    ...
    # === GAP 2: Simple decrement ordering — should use BEF_DECR pattern ===
    # Deaths and lapses come from pols_if directly.
    # Fix: maturities first → remove → add new biz → deaths → lapses from survivors.
    af.pols_if = (
    ...
    af.claim_pp_maturity = (
        when(af.has_gmab)
        .then(
            when(af.av_pp > af.sum_assured_f)
            .then(af.av_pp)
            .otherwise(af.sum_assured_f)
        )
        .otherwise(af.av_pp)
    ...
    # === GAP 4: Discount factors via cum_prod — should use closed-form ===
    # Cumulative product of per-period factors. Gives different results from
    # lifelib's closed-form (1+r)^(-t) when rates change between years.
    # Fix: disc_factors = exp(-month * ln(1 + disc_rate_mth))
