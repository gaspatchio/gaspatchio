...
`gaspatchio.scenarios` and folded by `run_aggregated`.
...
```bash
uv run python \
    bindings/python/gaspatchio/tutorials/patterns/period-aggregators/01_period_sum_mean.py
```
...
- **`PeriodCTE` `level` is a tail probability, not a confidence level.** The
  ...
  `gaspatchio/scenarios/_sketch.py` `SignedSketch.cte` — it samples
