...
from gaspatchio.polars_backend._shared import _unwrap_proxy
...
def accumulate(initial: pl.Expr, multiply: pl.Expr, add: pl.Expr) -> pl.Expr:
    ...
    --------
    >>> import polars as pl
    >>> from gaspatchio.polars_backend import accumulate
    >>> df = pl.DataFrame({
    ...     "initial": [100.0],
...
def list_pow(base: pl.Expr, exp: pl.Expr) -> pl.Expr:
    ...
    Examples:
        >>> import polars as pl
        >>> from gaspatchio.polars_backend import list_pow
        >>>
        >>> # List ** List
        >>> df = pl.DataFrame(
...
def list_clip(values: pl.Expr, lower: pl.Expr, upper: pl.Expr) -> pl.Expr:
    ...
    Examples:
        >>> import polars as pl
        >>> from gaspatchio.polars_backend import list_clip
        >>>
...
def list_conditional(
    ...
    Examples:
        >>> import polars as pl
        >>> from gaspatchio.polars_backend import list_conditional
        >>>
