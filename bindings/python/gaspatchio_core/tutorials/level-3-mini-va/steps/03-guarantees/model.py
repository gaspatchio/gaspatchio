...
"""
Level 3 → Step 03: Product Features — Guarantees & Surrender Charges

Adds GMDB/GMAB guarantee logic and surrender charges to claims.

Delta from Step 02:
  - SECTION 1: loads surrender_charges table; model points have guarantee flags
  - SECTION 5: added av_pp_mid_mth for claims timing
  - SECTION 7: rewritten with nested when/then for GMDB/GMAB + surrender charges
...
def load_assumptions():
    ...
    # NEW: Surrender charges by duration
...
def main(af: ActuarialFrame) -> ActuarialFrame:
    ...
    # NEW: Mid-month account value (used for claims — assumes decrement at mid-period)
    ...
    # =====================================================================
    # SECTION 7: CLAIMS (was: simple AV * decrements)
    # =====================================================================
    # NEW: GMDB guarantee, GMAB guarantee, surrender charges

    af.sum_assured_f = af.sum_assured.cast(pl.Float64)

    # Death claims: GMDB pays max(sum_assured, av_pp_mid_mth)
    af.claim_pp_death = (
        when(af.has_gmdb)
        .then(
    ...
    # Lapse claims: AV minus surrender charge
    ...
    # Maturity claims: GMAB pays max(sum_assured, av_pp)
