...
## 3. `until_value=99` — Truncated Projection

```python
# WRONG — truncates final year, ~3% BEL gap
af = af.projection.set(until="maximum_age", until_value=99, ...)

# RIGHT — projects through age 100; 99 drops the final high-mortality year
af = af.projection.set(until="maximum_age", until_value=100, ...)
```
...
# After af.projection.set(), stay lazy
af.mort_rate = table.lookup(age=af.age)
...
After `af.projection.set()`, columns created via Polars `with_columns` (rather than `af.col = expr`) are accessible via bracket notation but may fail with dot notation.
...
| `pl.col("x").cum_sum()` | `af.x.projection.accumulate(initial=0, multiply=af.x * 0.0 + 1.0, add=af.x)` — `multiply` must be list-shaped (a scalar `1.0` raises) | Yes |
