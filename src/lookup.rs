...
pub fn lookup(inputs: &[Series], kwargs: LookupKwargs) -> PolarsResult<Series> {
    ...
    info!(
        "Looking up values from table '{}' with {} keys",
        table_name,
        key_columns.len()
    );

    info!("Lookup values type: {:?}", lookup_values.dtype());
    if lookup_values.dtype().inner_dtype().is_some() {
        info!("Inner type: {:?}", lookup_values.dtype().inner_dtype());
    }
    info!("Sample lookup values: {:?}", lookup_values.head(Some(3)));

    for (i, key_col) in key_columns.iter().enumerate() {
        info!("Key column {} type: {:?}", i, key_col.dtype());
        info!("Sample key column {}: {:?}", i, key_col.head(Some(3)));
    }
        ...
        DataType::List(inner) => {
            ...
            match inner.as_ref() {
                ...
                _ => Err(PolarsError::ComputeError(
                    format!("Unsupported inner type in lookup function: {:?}", inner).into(),
                )),
            }
        }
        _ => Err(PolarsError::ComputeError(
